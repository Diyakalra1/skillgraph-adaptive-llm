...
def _make_prompt(obs, agent_id: str) -> str:
    ...
    return (
        ...
        "Respond in 1-2 concise lines with a useful move for this turn."
    )


def _generate_with_model(client: InferenceClient, model_id: str, prompt: str, max_tokens: int) -> str:
    ...
    try:
        ...
        try:
            ...
            # Keep training alive even when provider limits are hit.
            return (
                f"[FALLBACK_SIM] model={model_id} "
                f"chat_error={_first_error_line(chat_exc)} "
                f"text_error={_first_error_line(text_exc)}"
            )
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file, jsonl_path.open(
        ...
        for ep in range(1, episodes + 1):
            ...
            while not done and turn_guard < 24:
                ...
                response_text = _generate_with_model(client, model_id, prompt, max_tokens=max_tokens)
                if request_gap_s > 0:
